...
class TestSuite{
    ...
    constructor(testClass) {
        if(testClass){
            Object.getOwnPropertyNames(testClass.prototype).filter(m=> m.startsWith("test")).forEach(m=>{
                this.add(new testClass(m));
            })
        }
    }
...
const suite3 = new TestSuite(TestCaseTest);
const result3 = new TestResult();
suite3.run(result3);
console.log('suite3 :', result3.getSummary())
